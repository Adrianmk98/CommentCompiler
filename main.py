...
from Comment_parser.creds.import_reddit import fetch_reddit_creds
#from Comment_parser.dropdown.keybinds import open_keybinds_window

global reddit
reddit= fetch_reddit_creds()
from Comment_parser.load_recent.fetch_recent_posts import fetch_recent_posts


KEYBINDS_FILE = 'Comment_parser/dropdown/keybinds.ini'
...
def open_recent_posts_window(event=None):
    ...
    logo = tk.PhotoImage(file="Comment_parser/logoblue.png")
    recent_window.iconphoto(False, logo)
...
with open('Comment_parser/unparliamentary.txt', 'r', encoding='utf-8') as file:
...
drop_menu.add_command(label="Help", command=open_help_window)
...
# Set up the rest of your GUI here (same as you already have it)
# Example keybindings
root.bind("<Control-q>", move_to_next_user)
root.bind("<Control-s>", move_to_next_comment)
